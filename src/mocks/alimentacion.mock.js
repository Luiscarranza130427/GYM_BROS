...
/**
 * Alimentación simulada para desarrollar sin backend.
 *
 * La forma sale de `docs/MODELO_DE_DATOS.md`: cinco tablas encadenadas
 * `planes_alimentacion → comidas → comida_alimentos → alimentos`.
 *
 * Dos cosas que el esquema deja claras y conviene no perder de vista:
 *
 * 1. `alimentos` es un catálogo GLOBAL. No cuelga de `empresas`, al contrario
 *    que `ejercicios`, que sí tiene `id_empresas`. No asumir que funcionan igual.
 * 2. `planes_alimentacion` cuelga del USUARIO, no de la empresa. La empresa se
 *    arrastra desde el usuario sólo para poder filtrar.
 *
 * SUPUESTO, pendiente de confirmar con Natan: los macros de `alimentos` se
 * entienden por 100 g o 100 ml, salvo los de unidad `unidad`, que son por pieza.
 * El esquema no lo dice y sin fijarlo las cantidades no significan nada.
 */
 ...
 *
 * Los mocks no pueden depender de `new Date()` para decidir qué plan sigue
 * vigente: el mismo plan cambiaría de situación según el día en que se ejecute
 * y las pruebas empezarían a fallar solas con el tiempo.
 */
const HOY = new Date('2026-08-27T12:00:00')

/** Fecha como día local `AAAA-MM-DD`. No sirve `toISOString`: pasa a UTC. */
...
/**
 * Catálogo de alimentos: `[nombre, tipo, calorias, proteinas, carbohidratos,
 * grasas, fibra]` por 100 g/ml, o por pieza cuando se mide en unidades.
 */
const CATALOGO = [
...
/** Cómo se arma cada comida: qué alimentos y cuánto. Índices del CATALOGO. */
...
/** La unidad natural de un alimento según su tipo. */
...
/**
 * Macros de una porción.
 *
 * Los del catálogo son por 100 g/ml, así que la cantidad los escala. Sin este
 * factor, 30 g de aceite de oliva sumarían 884 kcal en vez de 265.
 */
...
/** Suma los macros de una lista de porciones ya calculadas. */
...
function construirComida(plantilla, orden) {
  ...
  return {
    ...
    // Derivados de sus alimentos, no inventados: si no cuadraran, la ficha
    // mostraría un total que no se corresponde con lo que tiene debajo.
...
function construirPlan(indice) {
  ...
  return {
    ...
    activo,
    /*
     * Los objetivos que fijó el entrenador, distintos de lo que suman las
     * comidas: la diferencia entre ambos es justo lo que hay que poder ver.
     *
     * Van anidados porque ésa es la forma que el SERVICIO garantiza. En modo
     * mock nadie normaliza —el mock es la respuesta—, así que emitir aquí los
     * cuatro campos sueltos rompería las vistas sólo con datos simulados.
     */
    objetivos: {
...
export async function obtenerAlimentosMock({
  ...
  // `usos` viaja también en el listado, no sólo en la ficha: el servicio lo
  // promete en la forma del alimento, y prometerlo sin entregarlo dejaría a las
  // tarjetas pintando un cero falso para todo el catálogo.
  return paginar(
...
function validarAlimento(payload, idActual = null) {
  ...
  if (Object.keys(errores).length > 0) {
    throw new HttpError({ status: 422, message: 'Revisa los datos introducidos.', errors: errores })
  }
}
...
/**
 * Simula DELETE /alimentos/:id
 *
 * Un alimento que ya forma parte de alguna comida no se retira: borrarlo dejaría
 * esas comidas apuntando a nada y sus macros dejarían de cuadrar. Es una regla
 * de negocio, no un campo mal rellenado, así que va como 422 SIN `errors`.
 */
export async function eliminarAlimentoMock(id) {
  ...
  if (usos > 0) {
    throw new HttpError({
      status: 422,
      message: `«${encontrado.nombre}» se usa en ${usos} ${usos === 1 ? 'comida' : 'comidas'} y no puede retirarse del catálogo.`,
    })
  }
...
/** Resumen de un plan para el listado: sin sus comidas, que no se muestran ahí. */
function resumirPlan(plan) {
  const { comidas, ...resto } = plan
  return { ...resto, totalComidas: comidas.length }
}

/** Simula GET /planes-alimentacion */
export async function obtenerPlanesMock({
  ...
  // Los vigentes primero: son sobre los que se actúa.
...
function buscarPlan(id) {
  ...
  if (!encontrado) {
    throw new HttpError({ status: 404, message: 'El plan solicitado no existe.' })
  }
  return encontrado
}

/**
 * Simula GET /planes-alimentacion/:id
 *
 * Las comidas salen ordenadas por hora, que es como se lee un horario. Con la
 * hora empatada decide el orden natural del día (`tipo_comida`), y luego el
 * campo `orden` que fijó el entrenador.
 */
export async function obtenerPlanMock(id) {
  await prepararRespuesta()
  const plan = buscarPlan(id)
  // `totalComidas` viaja también en la ficha: el servicio lo garantiza en las
  // dos formas y el resumen del plan lo pinta. Omitirlo aquí mostraba un 0 con
  // tres comidas debajo.
  return clonar({
    ...plan,
...
/** Recalcula los totales del plan a partir de sus comidas. */
...
/** Simula POST /planes-alimentacion/:id/comidas */
...
/** Simula PUT /planes-alimentacion/:idPlan/comidas/:idComida */
export async function actualizarComidaMock(idPlan, idComida, payload) {
  ...
  if (!anterior) {
    throw new HttpError({ status: 404, message: 'La comida solicitada no existe.' })
  }
  validarComida(payload)
...
/**
 * Simula DELETE /planes-alimentacion/:idPlan/comidas/:idComida
 *
 * Un plan sin ninguna comida no es un plan: dejarlo vacío daría una ficha que no
 * dice nada. Regla de negocio, de nuevo 422 SIN `errors`.
 */
...
/** Alimentos disponibles para elegir en una comida, sin paginar. */
...
/** Empresas con planes, para el filtro del listado. */
...
/** Restaura los fixtures; destinada a pruebas aisladas. */
