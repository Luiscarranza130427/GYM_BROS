...
/**
 * Catálogo de ejercicios simulado para desarrollar sin backend.
 *
 * Devuelve la misma forma que se espera de Laravel y lanza los mismos
 * `HttpError` (404, 422) que el interceptor real, de modo que al apagar los
 * mocks no cambia nada en las vistas.
 *
 * Los campos salen de lo único que el proyecto ya daba por cierto: los
 * `ejerciciosPopulares` del dashboard usan `{ nombre, categoria }`. El resto es
 * PROVISIONAL, pendiente de cerrar con Natan.
 */
...
/** Latencia variable para que los estados de carga sean visibles al desarrollar. */
...
function buscarEjercicio(id) {
  ...
  if (!encontrado) {
    throw new HttpError({ status: 404, message: 'El ejercicio solicitado no existe.' })
  }
  return encontrado
}

/**
 * Mismas reglas que valida el formulario, para que el mock rechace lo que
 * rechazaría Laravel y los estados de error se puedan probar de verdad.
 */
function validar(payload, idActual = null) {
  ...
  if (!EQUIPOS.includes(payload.equipo)) {
...
export async function obtenerEjerciciosMock({
  ...
  const items = filtrados.slice(desde, desde + porPagina)
...
export async function actualizarEjercicioMock(id, payload) {
  ...
  // `usos` y `fechaRegistro` los calcula el backend: no se dejan sobrescribir.
...
/** Simula DELETE /ejercicios/:id — desactivación lógica, igual que Empresas. */
