...
import { obtenerUsuarios } from '@/modules/usuarios/services/usuarios.service'
...
const usuarioSelect = useTemplateRef('usuarioSelect')
...
function validar(datos, errores) {
  ...
  if (datos.alcance === 'usuario' && !datos.idUsuarios) {
    errores.idUsuarios = 'Selecciona un usuario destinatario.'
  }
...
const { formulario, errorDe, limpiarError, validarParaEnviar } = useFormulario({
  ...
  referencias: {
    ...
    idUsuarios: usuarioSelect,
    fechaEnvio: fechaInput,
  },
  validar,
  alCargarValores(formulario, valores) {
    if (valores.idUsuarios) {
      formulario.alcance = 'usuario'
    } else if (valores.idEmpresas) {
...
const usuariosFiltrados = computed(() => {
  if (!formulario.idEmpresas) return listaUsuarios.value
  return listaUsuarios.value.filter((u) => Number(u.idEmpresas) === Number(formulario.idEmpresas))
})

const destinatarioTexto = computed(() => {
  if (formulario.alcance === 'todos') return 'Todos los usuarios'
  ...
  if (formulario.alcance === 'usuario') {
    const usu = listaUsuarios.value.find((u) => Number(u.id) === Number(formulario.idUsuarios))
    return usu ? `Usuario: ${usu.nombre}` : 'Usuario seleccionado'
  }
  return 'Destinatario'
})
...
onMounted(async () => {
  ...
  try {
    const [empresasRes, usuariosRes] = await Promise.all([
      obtenerEmpresas({ porPagina: 100 }),
      obtenerUsuarios({ porPagina: 100 }),
    ])
    listaEmpresas.value = empresasRes.items || []
    listaUsuarios.value = usuariosRes.items || []
  } catch {
    // Si falla la carga de empresas/usuarios, se mantiene el envío genérico
  } finally {
...
async function enviar() {
  ...
  const payload = {
    ...
    idEmpresas: formulario.idEmpresas ? Number(formulario.idEmpresas) : null,
    idUsuarios: formulario.idUsuarios ? Number(formulario.idUsuarios) : null,
  ...
  if (payload.idUsuarios) {
    const usu = listaUsuarios.value.find((u) => Number(u.id) === payload.idUsuarios)
    if (usu) payload.nombreUsuario = usu.nombre
  }
      ...
      <div class="notificacion-form__campos">
        ...
        <fieldset class="campo campo--completo notificacion-form__alcance-grupo">
          ...
          <div class="notificacion-form__alcance-opciones" role="radiogroup">
            <label
              v-for="opcion in DESTINATARIOS_ALCANCE"
              :key="opcion.valor"
        ...
        <!-- Filtros condicionales por empresa / usuario -->
        <div
          v-if="formulario.alcance === 'empresa' || formulario.alcance === 'usuario'"
          class="campo"
        >
          <label class="form-label" for="notif-empresa">
            Empresa / Sede {{ formulario.alcance === 'empresa' ? '*' : '(Filtrar usuarios)' }}
          </label>
          <select
            id="notif-empresa"
            ref="empresaSelect"
        ...
        <div v-if="formulario.alcance === 'usuario'" class="campo">
          <label class="form-label" for="notif-usuario">Usuario destinatario *</label>
          <select
            id="notif-usuario"
            ref="usuarioSelect"
            v-model="formulario.idUsuarios"
            class="form-select"
            :class="{ 'is-invalid': errorDe('idUsuarios') }"
            name="idUsuarios"
            :disabled="cargandoDestinatarios"
            :aria-invalid="Boolean(errorDe('idUsuarios'))"
            :aria-describedby="errorDe('idUsuarios') ? 'error-usuario' : null"
            @change="limpiarError('idUsuarios')"
          >
            <option value="">
              {{ cargandoDestinatarios ? 'Cargando usuarios…' : 'Selecciona un usuario' }}
            </option>
            <option v-for="usu in usuariosFiltrados" :key="usu.id" :value="usu.id">
              {{ usu.nombre }} ({{ usu.correo }})
            </option>
          </select>
          <p v-if="errorDe('idUsuarios')" id="error-usuario" class="campo__error" role="alert">
            {{ errorDe('idUsuarios') }}
          </p>
        </div>
