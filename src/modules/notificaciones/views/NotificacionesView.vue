<script setup>
import { AlertTriangle, CheckCircle2, Clock, History, Plus, Search } from 'lucide-vue-next'
...
watch(
  () => route.query.tab,
  (tab) => {
    if (tab && ['crear', 'programadas', 'enviadas'].includes(tab)) {
...
function duplicarNotificacion(notificacion) {
  valoresFormulario.value = {
    ...
    idUsuarios: notificacion.idUsuarios ? String(notificacion.idUsuarios) : '',
    programar: false,
    fechaEnvio: '',
  }
  ...
  <section class="notificaciones">
    ...
    <nav class="notificaciones__tabs" aria-label="Secciones de notificaciones">
      ...
      >
        ...
        <span
          v-if="pestana.id === 'programadas' && programadas.length > 0"
    ...
    <div class="notificaciones__panel-contenido">
      <!-- 1. Pestaña Crear / Redactar -->
      <section v-if="pestanaActiva === 'crear'" aria-label="Redactar nueva notificación">
        <NotificacionForm
