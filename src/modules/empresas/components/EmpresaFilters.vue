...
<template>
  <section class="filtros gb-tarjeta" aria-label="Filtros de empresas" :aria-busy="cargando">
    ...
    <fieldset class="filtros__estados">
      <legend class="visually-hidden">Filtrar por estado</legend>
      <label v-for="opcion in ['all', 'active', 'inactive']" :key="opcion">
        <input
        ...
        />
        <span>{{ { all: 'Todas', active: 'Activas', inactive: 'Inactivas' }[opcion] }}</span>
      </label>
    </fieldset>
  </section>
</template>
