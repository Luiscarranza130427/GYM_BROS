...
async function completar(wrapper) {
  ...
  await wrapper.get('#ejercicio-categoria').setValue('piernas')
  await wrapper.get('#ejercicio-nivel').setValue('intermedio')
  await wrapper.get('#ejercicio-equipo').setValue('barra')
  await wrapper.get('#ejercicio-series').setValue('4')
  await wrapper.get('#ejercicio-repeticiones').setValue('10')
}
...
describe('EjercicioCreateView', () => {
  it('crea y lleva al detalle anunciando el alta', async () => {
    ...
    expect(crearEjercicio).toHaveBeenCalledWith(
      expect.objectContaining({ nombre: 'Hip thrust', categoria: 'piernas' }),
    )
