...
describe('PagosView.vue', () => {
  ...
  it('actualiza los filtros cuando se selecciona una empresa', async () => {
    const wrapper = mount(PagosView)
    await flushPromises()

    const selectEmpresa = wrapper.find('#filtro-empresa')
    await selectEmpresa.setValue('1')
    await flushPromises()

    expect(mockReplace).toHaveBeenCalledWith(
      expect.objectContaining({
        query: expect.objectContaining({ empresaId: '1' }),
      }),
    )
  })
})
