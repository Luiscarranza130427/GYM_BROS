...
function diasRestantes(suscripcion) {
  if (!suscripcion || ['expired', 'none'].includes(suscripcion.estado)) return '—'
  return Math.max(0, Number(suscripcion.diasRestantes) || 0)
}
</script>
      ...
      <table>
        ...
        <thead>
          <tr>
            ...
            <th scope="col" class="tabla__numero">Días</th>
        ...
        <tbody v-else>
          <tr v-for="usuario in items" :key="usuario.id">
            ...
            <td><UsuarioSubscriptionBadge :estado="usuario.suscripcion?.estado" /></td>
            <td class="tabla__numero tabla__tabular">
              {{ diasRestantes(usuario.suscripcion) }}
            </td>
