...
describe('rutas del panel', () => {
  ...
  it('no registra como ruta simple ningún módulo con árbol propio', () => {
    // La exclusión se deriva de `arbolPropio` en navegacion.js, no de una lista
    // de nombres escrita a mano que hubiera que tocar con cada módulo nuevo.
    ...
    for (const seccion of conArbolPropio) {
      ...
      // `router.resolve` NO sigue los redirects: devuelve el registro padre. Lo
      // que se comprueba es que ese padre sea el nodo del árbol CRUD —redirige a
      // su listado y tiene hijos— y no una ruta simple con «En construcción».
      ...
      // Y el listado al que apunta existe de verdad.
